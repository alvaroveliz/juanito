...
function parseContent(html)
{
    ...
    $('[data-loop]', html).each(function(k, item){
        ...
        $('[data-loop-title]', item).each(function(t, title){
            $(title).html('<?php echo get_the_title(); ?>').removeAttr('data-loop-title');
        });
        ...
        $('[data-loop-author]', item).each(function(a, author){
            ...
            $(author).removeAttr('data-loop-author');
        });

        $('[data-loop-date]', item).each(function(d, date){
            $(date).html('<?php echo get_the_date(); ?>').removeAttr('data-loop-date');
        });
        ...
        $('[data-loop-excerpt]', item).each(function(e, excerpt){
            ...
            $(excerpt).removeAttr('data-loop-excerpt');
        });
        ...
        $('[data-loop-content]', item).each(function(c, content){
            ...
            $(content).removeAttr('data-loop-content');
        });

        $('[data-loop-permalink]', item).each(function(p, permalink){
            if (permalink.name = 'a') {
                $(permalink).attr('href', "<?php the_permalink(); ?>").html($(permalink).html());
            } else {
                $(permalink).html('<?php the_permalink(); ?>');
            }
            $(permalink).removeAttr('data-loop-permalink');
        });

        $('[data-loop-category]', item).each(function(c, category){
            $(category).html('<?php the_category(); ?>').removeAttr('data-loop-category');
        });

        $('[data-loop-tags]', item).each(function(t, tags){
            $(tags).html('<?php the_tags(); ?>').removeAttr('data-loop-tags');
        });
        ...
        $('[data-loop-thumbnail]', item).each(function(i, img){
            ...
            $(img).removeAttr('data-loop-thumbnail');
        });
        ...
        if (the_loop_name != '') {
            ...
            the_loop += '  if ($'+the_loop_name+'->have_posts()) : while ($'+the_loop_name+'->have_posts()) : $'+the_loop_name+'->the_post();\r\n';
            the_loop += '?>\r\n';
            the_loop += $(item);
            the_loop += '<?php endwhile; endif; wp_reset_postdata(); ?>';
        }
        else {
            the_loop += '\r\n<?php \r\n';
    ...
    $('img[src]', html).each(function(k, img){
        ...
        if (!$(img).attr('src').match(/^http([s]?):\/\/.*/)) {
            $(img).attr('src', '<?php echo get_template_directory_uri(); ?>/'+ $(img).attr('src'));    
        }
    });

    $('script[src]', html).each(function(k, scrpt){
        if (!$(scrpt).attr('src').match(/^http([s]?):\/\/.*/)) {
            $(scrpt).attr('src', '<?php echo get_template_directory_uri(); ?>/'+ $(scrpt).attr('src'));
        }
    });

    $('style[href]', html).each(function(k, style){
        if (!$(style).attr('href').match(/^http([s]?):\/\/.*/)) {
            $(style).attr('href', '<?php echo get_template_directory_uri(); ?>/'+ $(style).attr('href'));
        }
    });
        ...
        if (is_index) {
            ...
            if ($('[data-menu]').length > 0) {
                $('[data-menu]').each(function(m, menu) {
                    ...
                    all_class = $(menu).attr('class');
                    container_id = $(menu).attr('id');
                    ...
                    if ( menu_container.length ) {
                        menu_class = menu_container.attr('class');
                        menu_id = menu_container.attr('id');
                        tag_name = menu_container.get(0).tagName;
                    }

                    //menu_tpl = "" +
                    menu_tpl = "\r\n<?php\r\n";
                    menu_tpl += "   $opts['theme_location'] = '" + menu_name + "';\r\n";
                    menu_tpl += "   $opts['menu'] = '" + menu_name + "';\r\n";
                    menu_tpl += "   $opts['container'] = '" + tag_name + "';\r\n";
                    menu_tpl += "   $opts['container_class'] = '" + all_class + "';\r\n";
                    menu_tpl += "   $opts['container_id'] = '" + container_id + "';\r\n";
                    menu_tpl += "   $opts['menu_class'] = '" + menu_class + "';\r\n";
                    menu_tpl += "   $opts['menu_id'] = '" + menu_id + "';\r\n";
            ...
            header += ' <title><?php wp_title(); ?></title>\r\n';
